...
func object(v interface{}) gs.Object {
	switch v := v.(type) {
	case gs.Object:
		return v
	case string:
	...
	case time.Time:
		return &gs.Time{Value: v}
	case []int:
		var objs []gs.Object
		for _, e := range v {
			objs = append(objs, &gs.Int{Value: int64(e)})
		}
