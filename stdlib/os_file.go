...
func makeOSFile(vm core.VM, file *os.File) *value.Record {
	fileChdir := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 0 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.chdir", "0", len(args))
		}
	...
	fileClose := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 0 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.close", "0", len(args))
		}
	...
	fileSync := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 0 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.sync", "0", len(args))
		}
	...
	fileName := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 0 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.name", "0", len(args))
		}
		s := file.Name()
		if len(s) > core.MaxStringLen {
			return core.NewUndefined(), core.NewStringLimitError("os.file.name")
		}
	...
	fileChown := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 2 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.chown", "2", len(args))
		}
		i1, ok := args[0].AsInt()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.chown", "first", "int(compatible)", args[0].TypeName())
		}
		i2, ok := args[1].AsInt()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.chown", "second", "int(compatible)", args[1].TypeName())
		}
	...
	fileWrite := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 1 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.write", "1", len(args))
		}
		y1, ok := args[0].AsBytes()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.write", "first", "bytes(compatible)", args[0].TypeName())
		}
	...
	fileRead := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 1 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.read", "1", len(args))
		}
		y1, ok := args[0].AsBytes()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.read", "first", "bytes(compatible)", args[0].TypeName())
		}
	...
	fileWriteString := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 1 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.write_string", "1", len(args))
		}
		s1, ok := args[0].AsString()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.write_string", "first", "string(compatible)", args[0].TypeName())
		}
	...
	fileReaddirnames := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 1 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.read_dir_names", "1", len(args))
		}
		i1, ok := args[0].AsInt()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.read_dir_names", "first", "int(compatible)", args[0].TypeName())
		}
		...
		for _, r := range res {
			if len(r) > core.MaxStringLen {
				return core.NewUndefined(), core.NewStringLimitError("os.file.read_dir_names")
			}
	...
	fileChmod := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 1 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.chmod", "1", len(args))
		}
		i1, ok := args[0].AsInt()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.chmod", "first", "int(compatible)", args[0].TypeName())
		}
	...
	fileSeek := func(vm core.VM, args ...core.Value) (core.Value, error) {
		if len(args) != 2 {
			return core.NewUndefined(), core.NewWrongNumArgumentsError("os.file.seek", "2", len(args))
		}
		i1, ok := args[0].AsInt()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.seek", "first", "int(compatible)", args[0].TypeName())
		}
		i2, ok := args[1].AsInt()
		if !ok {
			return core.NewUndefined(), core.NewInvalidArgumentTypeError("os.file.seek", "second", "int(compatible)", args[1].TypeName())
		}
		res, err := file.Seek(i1, int(i2))
		if err != nil {
			return wrapError(vm, err), nil
		}
		return core.NewInt(res), nil
	}
